...
<div id="section-pos" class="lg:h-full flex flex-col gap-6 min-h-0" data-permission="pos.access">
    ...
    <div class="lg:flex-1 lg:min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white border border-cream-dark rounded-2xl flex flex-col justify-between overflow-hidden shadow hover:shadow-lg transition-shadow lg:h-full lg:min-h-0">
            ...
            <div class="p-4 border-b border-cream-dark shrink-0 space-y-3">
                ...
                <!-- Customer Selection -->
                <div>
                    <label class="text-[10px] font-bold text-zinc-400 block mb-1.5">Customer / Level Harga</label>
                    <select id="pos-customer-select" onchange="updatePOSCartPricing()" class="bg-cream border border-cream-dark text-sm rounded-3xl px-4 py-2.5 w-full font-bold focus:outline-none focus:ring-2 focus:ring-primary focus:bg-white">
            ...
            <div class="p-4 border-t border-cream-dark bg-white shrink-0 space-y-4">
                <!-- Diskon Item / Transaksi -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div class="bg-cream/40 p-3 rounded-2xl border border-cream-dark">
                        <span class="text-[10px] font-bold text-zinc-400 block uppercase mb-1">Diskon Transaksi</span>
                        <div class="flex items-center gap-1.5 mt-1">
                            <span class="text-xs font-bold text-zinc-600">%</span>
                            <input type="number" id="pos-discount-input" onkeyup="recalculatePOSCartTotal()" value="0" min="0" max="100" class="bg-transparent border-none p-0 text-sm font-bold w-full focus:outline-none">
                        </div>
                    </div>
                    <div class="bg-cream/40 p-3 rounded-2xl border border-cream-dark flex flex-col justify-center items-start">
                        <span class="text-[10px] font-bold text-zinc-400 block uppercase mb-1">Metode Default</span>
                        <span class="text-sm font-bold text-zinc-800 mt-1">Tunai / Cash</span>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center justify-between border-t border-dashed border-cream-dark pt-3">
                        <span class="font-extrabold text-sm">TOTAL AKHIR</span>
                        <span class="font-black text-base text-zinc-900" id="pos-cart-total-display">Rp 0</span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-2 pt-2 sm:grid-cols-3">
                    <!-- Hold bill -->
                    <button onclick="holdActiveBill()" class="bg-cream-dark hover:bg-zinc-200 text-zinc-800 font-bold py-3 rounded-2xl transition-all active:scale-95 flex items-center justify-center gap-2 text-xs">
                        <i data-lucide="archive" class="w-4 h-4"></i>
                        <span>Hold Bill</span>
                    </button>
                    <!-- Checkout / Pembayaran -->
                    <button onclick="openPaymentModal()" class="sm:col-span-2 bg-primary hover:bg-primary-dark text-zinc-900 font-black text-sm py-3 rounded-2xl shadow-md transition-all active:scale-95 flex items-center justify-center gap-2 border border-primary-dark/20" data-permission="pos.checkout">
                        <i data-lucide="credit-card" class="w-4 h-4 text-zinc-950"></i>
...
<script>
    ...
    function simulateBarcodeScan() {
        ...
        setTimeout(() => {
            // Check if products array exists in window scope or internal
            const product = products.find(p => p.barcode === '8991234560012');
            if (product) {
    ...
    function holdActiveBill() {
        ...
        let custName = customers[custIdx]?.name || 'Pelanggan';
        holdBills.push({
            id: Date.now(),
            customer: custName,
            cart: [...posCart],
